...
func (t *tester) InitialiseTest(cfg *TestConfig) (InitialiseTestReturn, error) {
	...
	// ask if these stoppers need to be reused in mockAssert
	select {
	case <-stopper:
	...
	default:
		// start the proxy
		returnVal.ProxySet = proxy.BootProxy(t.logger, proxy.Option{Port: cfg.Proxyport, MongoPassword: cfg.MongoPassword,BaseUrl: cfg.BaseUrl}, cfg.AppCmd, cfg.AppContainer, 0, "", cfg.PassThroughPorts, returnVal.LoadedHooks, context.Background())
	}
